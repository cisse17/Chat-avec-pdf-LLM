...
if not openai_api_key:
    st.error("La clé API OpenAI n'est pas définie. Veuillez la définir dans le fichier .env ou comme variable d'environnement.")
    st.stop()

# Chemin relatif au lieu d'absolu pour meilleure portabilité
...
def create_or_load_vector_store(chunks, file_name):
    """Crée ou charge un stockage vectoriel FAISS"""
    try:
        index_path = DOCUMENTS_DIR / f"{file_name}.faiss"
        ...
        if index_path.exists():
            try:
                ...
                st.warning(f"Impossible de charger l'index existant: {str(e)}. Un nouvel index sera créé.")
        
        embeddings = OpenAIEmbeddings(openai_api_key=openai_api_key)
        db = FAISS.from_texts(chunks, embedding=embeddings)
        db.save_local(str(DOCUMENTS_DIR), index_name=file_name)
        return db
    except Exception as e:
        st.error(f"Erreur lors de la création/chargement de l'index: {str(e)}")
        return None
...
def main():
    ...
    if pdf is not None:
        ...
        if not text:
            st.error("Aucun texte n'a pu être extrait du PDF. Vérifiez le format du fichier.")
            return
        ...
        if not chunks:
            st.error("Le découpage du texte n'a produit aucun chunk valide.")
            return

        file_name = pdf.name[:-4].replace(" ", "_").replace(".", "_")
        db = create_or_load_vector_store(chunks, file_name)
    ...
    if query and db is not None:
        ...
        st.warning("Veuillez d'abord télécharger un fichier PDF valide.")
